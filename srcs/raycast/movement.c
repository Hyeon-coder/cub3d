...
void	player_movement(t_game *game, double frame_time)
{
	...
	if (mlx_is_key_down(game->mlx_ptr, MLX_KEY_A))
	{
		...
		game->player.dir.x = game->player.dir.x * cos(-rotation_speed) - game->player.dir.y * sin(-rotation_speed);
        	game->player.dir.y = old_dir_x * sin(-rotation_speed) + game->player.dir.y * cos(-rotation_speed);
                old_plane_x = game->player.plane.x;
        	game->player.plane.x = game->player.plane.x * cos(-rotation_speed) - game->player.plane.y * sin(-rotation_speed);
        	game->player.plane.y = old_plane_x * sin(-rotation_speed) + game->player.plane.y * cos(-rotation_speed);
	}
